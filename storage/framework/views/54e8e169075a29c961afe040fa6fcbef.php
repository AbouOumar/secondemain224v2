...
<span class="badge position-absolute top-0 end-0 m-2 bg-<?php echo e($item->statut === 'vendu' ? 'danger' : ($item->is_boosted ? 'warning' : 'success')); ?>"><?php echo e($item->statut === 'vendu' ? 'Vendu' : ($item->is_boosted ? 'Boosté' : 'En vente')); ?></span>
<?php if($item->is_boosted): ?><span class="badge position-absolute top-0 start-0 m-2 bg-warning text-dark"><i class="bx bx-rocket"></i></span><?php endif; ?>
</div>
...
<span class="small text-muted mb-2"><?php echo e($item->category->libelle ?? ''); ?></span>
            <div class="d-flex gap-2 flex-wrap">
                ...
                <button type="button" class="btn btn-sm btn-outline-warning" data-url="<?php echo e(route('articles.boost', $item->id)); ?>" onclick="openBoostModal(this)"><i class="bx bx-rocket"></i> Booster</button>
                <?php else: ?>
