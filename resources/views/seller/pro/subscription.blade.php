            ...
            <div class="card border-0 shadow-sm">
                ...
                <div class="card-body p-4">
                            ...
                            <div class="row g-4">
                                <div class="col-md-6">
                                    <div class="border rounded p-3">
                                        <h6 class="mb-2">{{ ucfirst(auth()->user()->subscription->plan_type) }}</h6>
                                        <p class="mb-2"><strong>Statut :</strong> 
                                            <span class="badge bg-{{ auth()->user()->subscription->isActive() ? 'success' : 'danger' }}">
                                                {{ ucfirst(auth()->user()->subscription->status) }}
                                            </span>
                                        </p>
                                        <p class="mb-1"><strong>Période :</strong> 
                                            {{ auth()->user()->subscription->starts_at->format('d/m/Y') }} - 
                                            {{ auth()->user()->subscription->ends_at->format('d/m/Y') }}
                                        </p>
                                        @if (auth()->user()->subscription->isActive())
                                            <form method="POST" action="{{ route('api.subscription.cancel') }}" class="d-inline">
                                                @csrf
                                ...
                                <div class="col-md-6">
                                    <div class="border rounded p-3">
                                        ...
                                        <ul class="list-unstyled mb-0">
                                            @if (auth()->user()->subscription->plan_type === 'basic')
                                            ...
                                            @elseif (auth()->user()->subscription->plan_type === 'pro')
                                            ...
                                            @elseif (auth()->user()->subscription->plan_type === 'enterprise')
                    ...
                    <div class="row g-4">
                        <!-- Basic Plan -->
                        <div class="col-md-4">
                            <div class="card border-0 shadow-sm h-100 border-{{ auth()->user()->subscription?->plan_type === 'basic' ? 'border-primary' : '' }}">
                                <div class="card-body p-4">
                                    ...
                                    @if (!auth()->user()->subscription || auth()->user()->subscription->plan_type !== 'basic')
                        ...
                        <!-- Pro Plan -->
                        <div class="col-md-4">
                            <div class="card border-0 shadow-sm h-100 border-{{ auth()->user()->subscription?->plan_type === 'pro' ? 'border-primary' : '' }}">
                                <div class="card-body p-4">
                                    ...
                                    @if (!auth()->user()->subscription || auth()->user()->subscription->plan_type !== 'pro')
                        ...
                        <!-- Enterprise Plan -->
                        <div class="col-md-4">
                            <div class="card border-0 shadow-sm h-100 border-{{ auth()->user()->subscription?->plan_type === 'enterprise' ? 'border-primary' : '' }}">
                                <div class="card-body p-4">
                                    ...
                                    @if (!auth()->user()->subscription || auth()->user()->subscription->plan_type !== 'enterprise')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.select-plan').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            fetch('{{ route("api.subscription.store") }}', {
                method: 'POST',
                headers: {
