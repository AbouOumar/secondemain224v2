            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body p-4">
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <form id="statusForm">
                            <select class="form-select form-select-sm" name="status" id="statusSelect">
                                <option value="en_ligne" {{ auth()->user()->rider_status === 'en_ligne' ? 'selected' : '' }}>En ligne</option>
                                <option value="occupe" {{ auth()->user()->rider_status === 'occupe' ? 'selected' : '' }}>Occupé</option>
                                <option value="hors_ligne" {{ auth()->user()->rider_status === 'hors_ligne' ? 'selected' : '' }}>Hors ligne</option>
                            </select>
                    ...
                    <div class="alert alert-info small" id="statusMessage">
                        Votre statut actuel : <strong>{{ ucfirst(auth()->user()->rider_status) }}</strong>
                    </div>
                </div>
            </div>
        </div>
                                ...
                                <div class="list-group-item border-0 pb-3 pt-2">
                                    <div class="d-flex justify-content-between align-items-start">
                                        <div>
                                            ...
                                            @if($delivery->order->article->delivery_prix)
                                                <span class="badge bg-info text-white fs-6">
                                                    Livraison : {{ number_format($delivery->order->article->delivery_prix, 0, ',', ' ') }} GNF
                                                </span>
                                            @endif
                                        </div>
                                        <div class="text-end">
                                ...
                                <tbody>
                                    @foreach($historyDeliveries as $delivery)
                                        <tr>
                                            ...
                                            <td class="align-middle">{{ $delivery->order->buyer->name ?? 'Client' }}</td>
                                            <td class="align-middle">{{ $delivery->order->seller->name ?? 'Vendeur' }}</td>
                                            <td class="align-middle">{{ $delivery->completed_at->format('d/m/Y H:i') }}</td>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (statusForm) {
        statusForm.addEventListener('submit', function(e) {
            ...
            fetch('{{ route('deliveries.setStatus') }}', {
                method: 'POST',
                headers: {
