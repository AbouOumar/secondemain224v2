    ...
    <div class="row g-0 h-100">
        <div class="col-md-4 col-lg-3 p-3 bg-light overflow-auto" style="max-height: 100%;">
            <div class="d-flex align-items-center mb-3">
                <a href="{{ route('motard.dashboard') }}" class="btn btn-outline-secondary btn-sm me-2">
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-body">
                    ...
                    <div class="mb-2">
                        ...
                        <strong>{{ $delivery->pickup_adresse }}</strong>
                    </div>
                    <div class="mb-2">
                        <small class="text-muted d-block">À (livraison)</small>
                        <strong>{{ $delivery->delivery_adresse }}</strong>
                    </div>
                    <div class="mb-2">
                        <small class="text-muted d-block">Client</small>
                        <strong>{{ $delivery->order->buyer->name ?? 'Client' }}</strong>
                    </div>
                    <div>
                        <small class="text-muted d-block">Gain</small>
                        <strong class="text-success">{{ number_format($delivery->prix, 0, ',', ' ') }} GNF</strong>
                    </div>
                </div>
            </div>
            ...
            <div class="card border-0 shadow-sm mb-3">
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('deliveries.pickup', $delivery->id) }}">
                        @csrf
...
let trackPoints = @json($track ?? []);
const deliveryId = {{ $delivery->id }};
const pickupLat = {{ $delivery->pickup_latitude }};
const pickupLng = {{ $delivery->pickup_longitude }};
const deliveryLat = {{ $delivery->delivery_latitude }};
const deliveryLng = {{ $delivery->delivery_longitude }};
const pickupAddress = {!! json_encode($delivery->pickup_adresse) !!};
const deliveryAddress = {!! json_encode($delivery->delivery_adresse) !!};
...
function sendPosition(lat, lng) {
    fetch('{{ route("motard.position", $delivery->id) }}', {
        method: 'POST',
        headers: {
