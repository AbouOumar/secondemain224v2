...
<img src="{{ asset('assets/img/hero-bg.jpg') }}" alt="hero" style="max-width:100%;border-radius:10px;box-shadow:var(--card-shadow);height:190px;object-fit:cover;">
</div>
</div>
</div>
</section>
...
<p class="card-text text-muted small mb-2">{{ $article->user->name }} • {{ $article->localisation }}</p>
<p class="card-text fw-bold mb-3">{{ number_format($article->prix, 0, ',', ' ') }} {{ $article->currency?->value }}</p>
<div class="d-flex justify-content-between align-items-center mt-auto">
<a href="{{ route('articles.show', $article->slug) }}" class="btn btn-sm btn-outline-primary">Voir détails</a>
                    <a href="{{ route('magasin.show', $article->user->partner->slug) }}" class="badge bg-success text-decoration-none">{{ $article->user->partner->nom_magasin }}</a>
</div>
</div>
</div>
</div>
...
<div class="cat-item text-center flex-shrink-0" onclick="selectCategory('{{ $cat->id }}')">
<div class="cat-circle"><i class="bx {{ $cat->icon }}"></i></div>
<div class="mt-1 small">{{ $cat->libelle }}</div>
</div>
@endforeach
</div>
</section>
